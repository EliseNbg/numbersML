...
def algorithm_payload():
    return {
        "name": "Test Algorithm",
        "description": "Test algorithm for API tests",
        "mode": "paper",
        "config": {
...
class TestAuthorization:
    ...
    def test_missing_api_key_returns_401(self):
        response = client.post("/api/algorithms", json={"name": "Test"})
    ...
    def test_read_key_cannot_create_algorithm(self, read_headers, algorithm_payload):
        response = client.post("/api/algorithms", json=algorithm_payload, headers=read_headers)
        assert response.status_code == 403
        assert "Trader or admin access required" in response.json()["detail"]

    def test_trader_key_can_create_algorithm(self, trader_headers, algorithm_payload):
        with patch("src.infrastructure.api.routes.algorithms.get_algorithm_repo") as mock_repo:
            mock_repo.return_value.save = AsyncMock()
            mock_repo.return_value.create_version = AsyncMock()
            response = client.post("/api/algorithms", json=algorithm_payload, headers=trader_headers)
...
class TestAlgorithmCRUD:
    ...
    def test_create_algorithm_success(self, trader_headers, algorithm_payload):
        with patch("src.infrastructure.api.routes.algorithms.get_algorithm_repo") as mock_repo:
            mock_save = AsyncMock()
            ...
            assert "algorithm" in response.json()["message"].lower()

    def test_create_algorithm_invalid_payload(self, trader_headers):
        invalid_payload = {"name": ""}  # Empty name
        response = client.post("/api/algorithms", json=invalid_payload, headers=trader_headers)
        assert response.status_code == 422  # Validation error

    def test_list_algorithms(self, trader_headers):
        with patch("src.infrastructure.api.routes.algorithms.get_algorithm_repo") as mock_repo:
            mock_repo.return_value.get_all = AsyncMock(return_value=[])
            response = client.get("/api/algorithms", headers=trader_headers)
            assert response.status_code == 200
            assert isinstance(response.json(), list)

    def test_get_algorithm_not_found(self, trader_headers):
        with patch("src.infrastructure.api.routes.algorithms.get_algorithm_repo") as mock_repo:
            mock_repo.return_value.get_by_id = AsyncMock(return_value=None)
            response = client.get(
                "/api/algorithms/123e4567-e89b-12d3-a456-426614174000",
...
class TestAlgorithmLifecycle:
    """Test algorithm lifecycle endpoints."""

    def test_activate_algorithm_no_auth(self):
        response = client.post(
            "/api/algorithms/123e4567-e89b-12d3-a456-426614174000/activate",
            json={"version": 1},
        )
        assert response.status_code == 401

    def test_activate_algorithm_with_trader_auth(self, trader_headers):
        with (
            patch("src.infrastructure.api.routes.algorithms.get_lifecycle_service") as mock_svc,
            patch("src.infrastructure.api.routes.algorithms.get_algorithm_repo") as mock_repo,
        ):
            mock_repo.return_value.get_by_id = AsyncMock(return_value=MagicMock(mode="paper"))
            mock_svc.return_value.activate_algorithm = AsyncMock(return_value=True)
            response = client.post(
                "/api/algorithms/123e4567-e89b-12d3-a456-426614174000/activate",
                json={"version": 1},
                headers=trader_headers,
...
class TestLLMEndpoints:
    ...
    def test_generate_algorithm_no_auth(self):
        response = client.post(
            "/api/algorithms/generate",
            json={"description": "Test algorithm"},
        )
        assert response.status_code == 401

    @patch("src.infrastructure.api.routes.algorithms.LLMAlgorithmService")
    def test_generate_algorithm_success(self, mock_llm, trader_headers):
        ...
        mock_instance.save_generated_algorithm = AsyncMock(
        ...
        response = client.post(
            "/api/algorithms/generate",
            json={"description": "Create RSI algorithm for BTC", "symbols": ["BTC/USDC"]},
            headers=trader_headers,
        )
        assert response.status_code == 200
        assert "generated successfully" in response.json()["message"].lower()

    def test_generate_algorithm_invalid_description(self, trader_headers):
        response = client.post(
            "/api/algorithms/generate",
