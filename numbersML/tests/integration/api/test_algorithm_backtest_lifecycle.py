"""
Integration tests for Algorithm Backtest API lifecycle.
...
class TestBacktestLifecycle:
    ...
    def test_submit_and_check_job_status(self, client, trader_headers, backtest_job_payload):
        ...
        # Check job status
        status_resp = client.get(f"/api/algorithm-backtests/jobs/{job_id}", headers=trader_headers)
    ...
    def test_list_jobs(self, client, read_headers):
        ...
        resp = client.get("/api/algorithm-backtests/jobs", headers=read_headers)
...
class TestBacktestAuth:
    ...
    def test_submit_requires_auth(self, client, backtest_job_payload):
        resp = client.post("/api/algorithm-backtests/jobs", json=backtest_job_payload)
        assert resp.status_code == 401

    def test_read_requires_auth(self, client):
        resp = client.get("/api/algorithm-backtests/jobs/some-job-id")
        assert resp.status_code == 401
...
class TestBacktestErrors:
    ...
    def test_get_nonexistent_job_returns_404(self, client, read_headers):
        ...
        resp = client.get(f"/api/algorithm-backtests/jobs/{fake_job_id}", headers=read_headers)
        assert resp.status_code == 404
