...
class TestSyntheticPipelineIntegration:
    ...
    async def test_full_pipeline_time_alignment(self, db_pool: asyncpg.Pool) -> None:
        ...
        t02_candles = generate_sine_candles(base_time, duration_hours=2, base_price=50.0, amplitude=2.5)
        ...
        async with db_pool.acquire() as conn:
            ...
            for row in candle_counts:
                assert row['count'] >= 7000, f"Expected at least 7000 candles for {row['symbol']}, got {row['count']}"
            ...
            for row in indicator_counts:
                assert row['count'] >= 7000, f"Expected at least 7000 indicators for {row['symbol']}, got {row['count']}"
            ...
            assert match_ratio_cw > 0.98, f"Low candle-wide vector match ratio: {match_ratio_cw:.2%}"
            # Indicators may have up to 1 second processing delay
            assert match_ratio_iw > 0.95, f"Low indicator-wide vector match ratio: {match_ratio_iw:.2%}"
            ...
            assert sample_vector['vector_size'] >= 52, f"Expected vector size >= 52, got {sample_vector['vector_size']}"
            assert len(sample_vector['column_names']) == sample_vector['vector_size'], "Column names length mismatch"
            assert len(sample_vector['symbols']) >= 4, f"Expected at least 4 symbols in vector, got {len(sample_vector['symbols'])}"
