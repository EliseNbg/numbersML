...
async def wait_for_enrichment(
    conn: asyncpg.Connection, symbol_ids: list[int], timeout: float = ENRICHMENT_TIMEOUT
) -> bool:
    """
    ...
    while (datetime.now(UTC) - start_time).total_seconds() < timeout:
        try:
            notification = await asyncio.wait_for(conn.notification(), timeout=1.0)
...
async def _get_latest_ticks(conn: asyncpg.Connection, symbol_ids: list[int]) -> set[tuple]:
...
async def run_integration_test() -> dict[str, Any]:
    ...
    try:
        async with pool.acquire() as conn:
            ...
            symbol_rows = await conn.fetch("SELECT id, symbol FROM symbols WHERE is_test = true")
            ...
            if indicators:
                sample = await conn.fetchrow("""
                    ...
                    """)
                print(
                    f"  Sample: {sample['symbol']} has {len(sample['indicator_keys'])} indicators"
                )
            ...
            print(
                f"  {'✓' if validation['passed'] else '✗'} Validation: {'PASSED' if validation['passed'] else 'FAILED'}"
            )
    ...
    print(
        f"Validation: {'PASSED ✓' if results['validation'] and results['validation']['passed'] else 'FAILED ✗'}"
    )
