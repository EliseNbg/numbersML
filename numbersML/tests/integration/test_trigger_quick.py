...
async def test_trigger():
    ...
    try:
        async with pool.acquire() as conn:
            # Get test symbol ID
            symbol_row = await conn.fetchrow("SELECT id FROM symbols WHERE symbol = $1", "ts1/USDC")
            ...
            # Insert 60 candles to build up price history
            print("\nInserting 60 candles to build price history...")
            base_price = 100.0
            ...
            for i in range(60):
                ...
                await conn.execute(
                    ...
                    INSERT INTO candles_1s (
                        ...
                        NOW() - INTERVAL '60 seconds' + INTERVAL '{i} seconds',
                        $1, $2, $3, $4, $5, $6, $7, 100
                    )
                    """,
                    symbol_id,
            ...
            print(f"Candles inserted: {candle_count}")
