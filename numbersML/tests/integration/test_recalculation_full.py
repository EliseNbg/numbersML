...
async def insert_synthetic_candles(conn: asyncpg.Connection, symbol_id: int) -> datetime:
    ...
    logger.info(f"Created exactly {count} candles from {time_range['min_time']} to {time_range['max_time']}")
...
async def validate_exact_counts(conn: asyncpg.Connection, symbol_id: int):
    ...
    assert candle_count == TEST_CANDLE_COUNT, f"Expected {TEST_CANDLE_COUNT} candles, got {candle_count}"
    ...
    if sample_row:
        ...
        logger.info(f"Validation: {candle_count} candles × {total_values} values = {expected_total} total values")
...
async def validate_time_alignment(conn: asyncpg.Connection, symbol_id: int):
    ...
    for i, (candle, indicator) in enumerate(zip(candle_times, indicator_times)):
        ...
        # Verify UTC timezone
        assert candle['time'].tzinfo == timezone.utc, f"Candle time {candle['time']} not in UTC"
        assert indicator['time'].tzinfo == timezone.utc, f"Indicator time {indicator['time']} not in UTC"
    
    logger.info(f"Perfect time alignment validated: {len(candle_times)} timestamps, 1:1 mapping, UTC")
...
async def validate_indicator_values(conn: asyncpg.Connection, symbol_id: int):
    ...
    logger.info(f"Indicator validation passed: MACD positive={macd_positive_count}, negative={macd_negative_count}")
