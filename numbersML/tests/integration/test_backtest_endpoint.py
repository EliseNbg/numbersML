...
def test_backtest_models_list_endpoint():
    """Test GET /api/backtest/models/entry endpoint returns valid response"""
...
def test_backtest_endpoint_unknown_symbol():
    ...
    response = client.get("/api/backtest/entry?symbol=UNKNOWN/XXX&model=test.pkl&seconds=86400")
    assert response.status_code == 200
    data = response.json()
