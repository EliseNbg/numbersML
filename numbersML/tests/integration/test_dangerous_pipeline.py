...
class DangerousIntegrationTest:
    ...
    async def stop_pipeline(self):
        ...
        try:
            subprocess.run(["pkill", "-f", "start_trade_pipeline"], capture_output=True, timeout=10)
    ...
    async def delete_all_data(self):
        ...
        async with self.db_pool.acquire() as conn:
            ...
            try:
                await conn.execute(
                    "UPDATE symbols SET is_active = true WHERE symbol IN ('BTC/USDC', 'ETH/USDC', 'ADA/USDC', 'DOGE/USDC')"
                )
    ...
    async def verify_candles(
        self, symbol_ids: list[int], start_time: datetime, end_time: datetime
    ) -> dict:
    ...
    def print_results(self, candle_results: dict, indicator_results: dict, vector_results: dict):
        ...
        for symbol_id, result in candle_results.items():
            ...
            print(
                f"  Total: {result['total']}, Distinct seconds: {result['distinct_seconds']}/{result['expected_seconds']}"
            )
        ...
        print(
            f"Total: {vector_results['total']}, Distinct seconds: {vector_results['distinct_seconds']}/{vector_results['expected_seconds']}"
        )
