...
class TestSymbolActivation:
    ...
    async def test_toggle_symbol_activation(self, db_pool: asyncpg.Pool) -> None:
        ...
        # Deactivate
        async with db_pool.acquire() as conn:
            await conn.execute("UPDATE symbols SET is_active = false WHERE symbol = $1", symbol)
...
class TestIndicatorActivation:
    ...
    async def test_toggle_indicator_activation(self, db_pool: asyncpg.Pool) -> None:
        ...
        async with db_pool.acquire() as conn:
            ...
            indicator_name = await conn.fetchval("SELECT name FROM indicator_definitions LIMIT 1")
            ...
            # Deactivate
            await conn.execute(
                "UPDATE indicator_definitions SET is_active = false WHERE name = $1", indicator_name
            )

            # Verify inactive
            is_active = await conn.fetchval(
                "SELECT is_active FROM indicator_definitions WHERE name = $1", indicator_name
            )
            assert is_active is False

            # Reactivate
            await conn.execute(
                "UPDATE indicator_definitions SET is_active = true WHERE name = $1", indicator_name
            )
    ...
    @pytest.mark.asyncio
    async def test_enrichment_service_loads_active_indicators(self, db_pool: asyncpg.Pool) -> None:
...
class TestPipelineMetrics:
    ...
    async def test_save_pipeline_metrics(self, db_pool: asyncpg.Pool) -> None:
        ...
        # Get a symbol ID
        async with db_pool.acquire() as conn:
            symbol_id = await conn.fetchval("SELECT id FROM symbols WHERE is_active = true LIMIT 1")
...
class TestDashboardViews:
    ...
    async def test_v_pipeline_performance_view(self, db_pool: asyncpg.Pool) -> None:
        ...
        async with db_pool.acquire() as conn:
            result = await conn.fetch("SELECT * FROM v_pipeline_performance LIMIT 1")
    ...
    async def test_v_active_configuration_view(self, db_pool: asyncpg.Pool) -> None:
        ...
        async with db_pool.acquire() as conn:
            result = await conn.fetchrow("SELECT * FROM v_active_configuration")
    ...
    async def test_v_sla_compliance_view(self, db_pool: asyncpg.Pool) -> None:
        ...
        async with db_pool.acquire() as conn:
            result = await conn.fetch("SELECT * FROM v_sla_compliance LIMIT 1")

            # View should exist
            assert result is not None

    @pytest.mark.asyncio
    async def test_get_pipeline_performance_function(self, db_pool: asyncpg.Pool) -> None:
        """Test get_pipeline_performance() helper function."""
        async with db_pool.acquire() as conn:
            result = await conn.fetchrow("SELECT * FROM get_pipeline_performance(5)")
    ...
    @pytest.mark.asyncio
    async def test_can_handle_more_symbols_function(self, db_pool: asyncpg.Pool) -> None:
        """Test can_handle_more_symbols() helper function."""
        async with db_pool.acquire() as conn:
            result = await conn.fetchrow("SELECT * FROM can_handle_more_symbols(800, 0.2)")
