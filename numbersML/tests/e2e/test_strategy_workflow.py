...
async def client():
    ...
    async with AsyncClient(
        transport=transport, base_url="http://test", headers={"X-API-Key": "trader-test-key"}
    ) as ac:
        yield ac
...
class TestStrategyWorkflow:
    ...
    async def test_complete_strategy_workflow(self, client, db_pool):
        ...
        response = await client.post(f"/api/strategies/{strategy_id}/validate", json={})
        ...
        response = await client.put(
            f"/api/strategies/{strategy_id}", json={"description": "Updated description"}
        )
        ...
        print("  ✓ Strategy confirmed deleted (404 on GET)")
    ...
    async def test_emergency_stop_workflow(self, client, db_pool):
        ...
        response = await client.post(
            f"/api/strategies/{strategy_id}/activate", json={"mode": "paper"}
        )
        assert response.status_code == 200
    ...
    async def test_strategy_delete_workflow(self, client, db_pool):
        ...
        response = await client.get(f"/api/strategies/{strategy_id}")
        assert response.status_code == 200
        print("  ✓ Strategy exists (GET 200)")

        # Delete the strategy
        response = await client.delete(f"/api/strategies/{strategy_id}")
        assert response.status_code == 204
        print("  ✓ Strategy deleted (DELETE 204)")

        # Verify it no longer exists
        response = await client.get(f"/api/strategies/{strategy_id}")
        assert response.status_code == 404
        print("  ✓ Strategy not found after deletion (GET 404)")

        # Verify delete is idempotent - deleting again should return 404
        response = await client.delete(f"/api/strategies/{strategy_id}")
        assert response.status_code == 404
        print("  ✓ Delete idempotent (DELETE 404 for non-existent)")
