...
from src.domain.strategies.base import EnrichedTick, SignalType, TimeFrame
...
class TestGridAlgorithmInit:
    ...
    def test_create_grid_strategy(self, grid_strategy):
        ...
        assert isinstance(grid_strategy.id, uuid4().__class__)
        assert grid_strategy.symbols == ["TEST/USDT"]
        assert grid_strategy.time_frame == TimeFrame.TICK
