...
class TestBulkFetchFunctions:
    ...
    async def test_bulk_fetch_historical_candles(self) -> None:
        ...
        mock_conn.fetch = AsyncMock(return_value=[
            ...
            {'symbol_id': 1, 'time': now - timedelta(seconds=99),
             'high': 102.0, 'low': 100.0, 'close': 101.0, 'volume': 11.0},
        ])
        ...
        result = await _bulk_fetch_historical_candles(
            mock_pool, [1], now - timedelta(hours=1), now
        )
        ...
        for indicator in indicators_to_test:
            ...
            for key in latest:
                if latest[key] is not None:
                    ...
                    assert abs(latest[key] - full_last) < 1e-10, \
