"""
Unit tests for LLMAlgorithmService.
...
from src.application.services.llm_algorithm_service import (
    LLMAlgorithmService,
)
...
@pytest.fixture
def mock_algorithm_repo():
    """Mock algorithm repository."""
    return AsyncMock(spec=AlgorithmRepository)


@pytest.fixture
def llm_service(mock_algorithm_repo):
    """Create LLMAlgorithmService with mocked dependencies."""
    return LLMAlgorithmService(
        algorithm_repository=mock_algorithm_repo,
...
@pytest.fixture
def valid_algorithm_config():
    """Valid algorithm config for testing."""
    return {
        "meta": {
            "name": "Test RSI Algorithm",
...
class TestPromptInjectionDetection:
    ...
    def test_allows_safe_descriptions(self, llm_service):
        """Should allow legitimate algorithm descriptions."""
        safe_texts = [
            "Create an RSI algorithm that buys when oversold",
            "MACD crossover with fast=12 slow=26",
            "Mean reversion using Bollinger Bands",
            "Composite algorithm combining RSI and MACD signals",
        ]
    ...
    @pytest.mark.asyncio
    async def test_blocks_injection_in_generate(self, llm_service, mock_algorithm_repo):
...
class TestConfigGeneration:
    ...
    @pytest.mark.asyncio
    async def test_returns_error_when_no_api_key(self, mock_algorithm_repo):
        """Should return error when OpenAI not configured."""
        service = LLMAlgorithmService(
            algorithm_repository=mock_algorithm_repo,
            api_key=None,
        )

        result = await service.generate_config(
            description="Create an RSI algorithm",
            symbols=["BTC/USDC"],
        )
    ...
    @pytest.mark.asyncio
    async def test_validates_generated_config(self, llm_service, valid_algorithm_config):
        """Should validate generated config against schema."""
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(message=MagicMock(content=json.dumps(valid_algorithm_config)))
        ]
...
class TestConfigModification:
    ...
    @pytest.mark.asyncio
    async def test_modifies_config_successfully(self, llm_service, valid_algorithm_config):
        """Should modify config based on change request."""
        modified_config = {**valid_algorithm_config}
        ...
        with patch.object(llm_service, "_client") as mock_client:
            ...
            result = await llm_service.modify_config(
                existing_config=valid_algorithm_config,
    ...
    def test_detects_forbidden_mode_change(self, llm_service, valid_algorithm_config):
        """Should detect unauthorized mode change to live."""
        new_config = {**valid_algorithm_config, "mode": "live"}

        issues = llm_service._check_forbidden_changes(
            old_config=valid_algorithm_config,
    ...
    def test_allows_explicit_live_change(self, llm_service, valid_algorithm_config):
    ...
    def test_detects_large_position_size_increase(self, llm_service, valid_algorithm_config):
        """Should flag large position size increases."""
        import copy

        new_config = copy.deepcopy(valid_algorithm_config)
...
class TestImprovementSuggestions:
    ...
    @pytest.mark.asyncio
    async def test_generates_suggestions(self, llm_service, valid_algorithm_config):
    ...
    @pytest.mark.asyncio
    async def test_handles_suggestion_error(self, llm_service, valid_algorithm_config):
...
# ============================================================================
# Save Generated Algorithm Tests
# ============================================================================


class TestSaveGeneratedAlgorithm:
    """Test saving LLM-generated algorithms."""

    @pytest.mark.asyncio
    async def test_saves_as_draft(self, llm_service, mock_algorithm_repo, valid_algorithm_config):
        """Should always save as draft status."""
        # Set status to active in config
        valid_algorithm_config["status"] = "active"
        valid_algorithm_config["mode"] = "live"

        mock_saved = AlgorithmDefinition(
            name="Test RSI Algorithm",
            description="Test",
            id=uuid4(),
        )
        mock_algorithm_repo.save = AsyncMock(return_value=mock_saved)
        mock_algorithm_repo.create_version = AsyncMock()

        saved = await llm_service.save_generated_algorithm(
            config=valid_algorithm_config,
            created_by="test",
        )

        # Should force to draft
        assert mock_algorithm_repo.save.call_args[0][0].status == "draft"

    @pytest.mark.asyncio
    async def test_creates_version_with_config(
        self, llm_service, mock_algorithm_repo, valid_algorithm_config
    ):
        ...
        await llm_service.save_generated_algorithm(
            config=valid_algorithm_config,
            created_by="test",
        )

        # Verify version created with config
        create_version_call = mock_algorithm_repo.create_version.call_args
        assert create_version_call.kwargs["config"] == valid_algorithm_config
...
class TestPromptBuilding:
    ...
    def test_generation_prompt_includes_schema(self, llm_service):
        ...
        prompt = llm_service._build_generation_prompt(
            description="RSI algorithm",
            symbols=["BTC/USDC", "ETH/USDC"],
            timeframe="5M",
            mode="paper",
        )

        assert "RSI algorithm" in prompt
    ...
    def test_modification_prompt_includes_existing_config(self, llm_service, valid_algorithm_config):
        ...
        assert '"meta"' in prompt and '"name": "Test RSI Algorithm"' in prompt
        assert "increase stop loss" in prompt
        assert "Never change mode from paper to live" in prompt

    def test_suggestion_prompt_includes_metrics(self, llm_service, valid_algorithm_config):
