...
def test_list_instances(client, mock_repo):
    ...
    response = client.get("/api/algorithm-instances")
...
def test_create_instance(client, mock_repo):
    ...
    response = client.post(
        ...
        json={"algorithm_id": str(algorithm_id), "config_set_id": str(config_set_id)},
    )

    assert response.status_code == 201
    assert response.json()["algorithm_id"] == str(algorithm_id)
...
def test_create_duplicate_instance(client, mock_repo):
    ...
    mock_repo.get_by_algorithm_and_config.return_value = existing

    response = client.post(
        "/api/algorithm-instances",
        json={
            "algorithm_id": str(existing.algorithm_id),
...
def test_get_instance(client, mock_repo):
    ...
    response = client.get(f"/api/algorithm-instances/{instance.id}")
...
def test_get_nonexistent_instance(client, mock_repo):
    ...
    response = client.get(f"/api/algorithm-instances/{uuid4()}")
...
def test_start_instance(client, mock_repo):
    ...
    response = client.post(f"/api/algorithm-instances/{instance.id}/start")
...
def test_start_nonexistent_instance(client, mock_repo):
    ...
    response = client.post(f"/api/algorithm-instances/{uuid4()}/start")
...
def test_stop_instance(client, mock_repo):
    ...
    response = client.post(f"/api/algorithm-instances/{instance.id}/stop")
...
def test_pause_instance(client, mock_repo):
    ...
    response = client.post(f"/api/algorithm-instances/{instance.id}/pause")
...
def test_resume_instance(client, mock_repo):
    ...
    response = client.post(f"/api/algorithm-instances/{instance.id}/resume")
...
def test_delete_instance(client, mock_repo):
    ...
    response = client.delete(f"/api/algorithm-instances/{uuid4()}")
