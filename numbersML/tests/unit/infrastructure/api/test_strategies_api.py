"""
Tests for Strategy (Algorithm) API endpoints.
...
from src.infrastructure.api.routes.strategies import router
...
@pytest.fixture
def mock_event_repo():
    """Mock StrategyRuntimeEventRepository."""
    return AsyncMock()


@pytest.fixture
def mock_lifecycle_svc():
    """Mock StrategyLifecycleService."""
    svc = MagicMock()
    ...
    svc.activate_strategy = AsyncMock(return_value=True)
    svc.deactivate_strategy = AsyncMock(return_value=True)
    svc.pause_strategy = AsyncMock(return_value=True)
    svc.resume_strategy = AsyncMock(return_value=True)
    return svc


@pytest.fixture
def mock_llm_svc():
    """Mock LLMStrategyService."""
    svc = AsyncMock()
    return svc
...
def client(app, mock_repo, mock_event_repo, mock_lifecycle_svc, mock_llm_svc, mock_auth):
    ...
    from src.infrastructure.api.routes.strategies import (
        get_event_repo,
    ...
    app.dependency_overrides[get_strategy_repo] = lambda: mock_repo
...
def sample_strategy_data():
    """Sample data for creating a strategy."""
    return {
        "name": "Test Strategy",
...
class TestCreateStrategy:
    ...
    def test_create_success(self, client, mock_repo, sample_strategy_data):
        ...
        strategy = StrategyDefinition(
            ...
            mode=sample_strategy_data["mode"],
        )
        mock_repo.save.return_value = strategy
...
class TestListStrategies:
    ...
    def test_list_with_data(self, client, mock_repo):
        ...
        strategy = StrategyDefinition(name="Test", description="Test desc", mode="paper")
        mock_repo.get_all.return_value = [strategy]
    ...
    def test_list_filter_by_status(self, client, mock_repo):
        ...
        strategy = StrategyDefinition(name="Test", description="Test desc", mode="paper", status="active")
        mock_repo.get_all.return_value = [strategy]
    ...
    def test_list_filter_by_mode(self, client, mock_repo):
        ...
        strategy = StrategyDefinition(name="Test", description="Test desc", mode="live")
        mock_repo.get_all.return_value = [strategy]

        response = client.get("/api/strategies?mode=live")
...
class TestGetStrategy:
    ...
    def test_get_nonexistent(self, client, mock_repo):
        ...
        response = client.get(f"/api/strategies/{uuid4()}")
...
class TestUpdateStrategy:
    ...
    def test_update_name(self, client, mock_repo):
        ...
        strategy = StrategyDefinition(name="Old", description="Old desc", mode="paper")
        mock_repo.get_by_id.return_value = strategy
        mock_repo.save.return_value = strategy
...
class TestStrategyVersions:
    ...
    def test_list_versions(self, client, mock_repo):
        ...
        response = client.get(f"/api/strategies/{uuid4()}/versions")
    ...
    def test_activate_version(self, client, mock_repo):
        ...
        response = client.post(
            f"/api/strategies/{uuid4()}/versions/1/activate",
        )
...
class TestLifecycleEndpoints:
    ...
    def test_activate_strategy(self, client, mock_lifecycle_svc):
        """Test activating a strategy."""
        response = client.post(
            f"/api/strategies/{uuid4()}/activate",
            json={"version": 1},
        )
    ...
    def test_deactivate_strategy(self, client, mock_lifecycle_svc):
        """Test deactivating a strategy."""
        response = client.post(
            f"/api/strategies/{uuid4()}/deactivate",
        )

        assert response.status_code == 200
        assert "deactivated" in response.json()["message"]

    def test_pause_strategy(self, client, mock_lifecycle_svc):
        """Test pausing a strategy."""
        response = client.post(
            f"/api/strategies/{uuid4()}/pause",
        )

        assert response.status_code == 200
        assert "paused" in response.json()["message"]

    def test_resume_strategy(self, client, mock_lifecycle_svc):
        """Test resuming a strategy."""
        response = client.post(
            f"/api/strategies/{uuid4()}/resume",
        )
...
class TestRuntimeState:
    ...
    def test_get_runtime_state(self, client, mock_lifecycle_svc):
        """Test getting runtime state for a strategy."""
        ...
        response = client.get(f"/api/strategies/{uuid4()}/runtime")
    ...
    def test_get_all_runtime_states(self, client, mock_lifecycle_svc):
        ...
        state1 = MagicMock()
        state1.strategy_id = uuid4()
        state1.strategy_name = "Test"
        ...
        response = client.get("/api/strategies/runtime")
...
class TestLifecycleEvents:
    ...
    def test_get_events(self, client, mock_lifecycle_svc):
        ...
        response = client.get(f"/api/strategies/{uuid4()}/events")
...
class TestLLMGeneration:
    ...
    def test_generate_strategy(self, client, mock_llm_svc):
        """Test generating strategy via LLM."""
        from src.application.services.llm_strategy_service import LLMGenerationResult
        from src.domain.strategies.config_schema import ValidationIssue
        ...
        # Mock save_generated_strategy
        from src.domain.strategies.strategy_config import StrategyDefinition

        strategy = StrategyDefinition(name="Generated", description="Generated desc", mode="paper")
        mock_llm_svc.save_generated_strategy = AsyncMock(return_value=strategy)

        response = client.post(
            "/api/strategies/generate",
            json={
                "description": "Create a simple moving average strategy",
                "symbols": ["BTC/USDC"],
            },
        )
    ...
    def test_generate_strategy_failure(self, client, mock_llm_svc):
        ...
        response = client.post(
            "/api/strategies/generate",
            json={
                "description": "This is a bad strategy description",
                "symbols": ["BTC/USDC"],
            },
        )
