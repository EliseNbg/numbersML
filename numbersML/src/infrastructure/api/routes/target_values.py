...
async def get_target_values(
    ...
    target_data_list = batch_calculate_target_data(
        prices, response_time=response_time, method=method, use_future=use_future
    )
...
async def calculate_target_values(
    ...
    async with db_pool.acquire() as conn:
        ...
        if hours is not None:
            ...
            from_dt = datetime.strptime(from_time, "%Y-%m-%d %H:%M:%S").replace(tzinfo=UTC)
            to_dt = (
                datetime.strptime(to_time, "%Y-%m-%d %H:%M:%S").replace(tzinfo=UTC)
                if to_time
                else now
            )
        else:
