...
async def run_trading_tcn_backtest(
    ...
    try:
        ...
        print(
            f"   Predictions: ret mean={predictions_ret.mean():.6f}, risk mean={predictions_risk.mean():.6f}"
        )
...
async def run_entry_backtest(
    ...
    model: str = Query(
        ..., description="Model filename (e.g., entry_model_BTC_USDC_20260418_2052.pkl)"
    ),
...
async def train_entry_model(
    ...
    loss: str = Query(
        "pnl", description="Loss function: pnl, sharpe, risk_adjusted (default: pnl)"
    ),
