...
class TradePipeline:
    ...
    async def _execute_ticket(self, ticket: PipelineTicket) -> None:
        ...
        if ticket.has(PipelineStep.STRATEGY):
            try:
                raw_msg = (
                    self._ws_manager.last_raw_message
                    if self._ws_manager
                    else None
                )
    ...
    async def _ticker_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                tick_time = datetime.now(UTC).replace(microsecond=0)
                ...
                if PipelineStep.WIDE_VECTOR in self._active_steps:
                    try:
                        ...
                        if result:
                            self._stats["wide_vectors_generated"] = (
                                self._stats.get("wide_vectors_generated", 0) + 1
                            )
                    except Exception as e:
                        logger.error(f"Wide vector error: {e}")
                        self._stats["wide_vector_errors"] = (
                            self._stats.get("wide_vector_errors", 0) + 1
                        )
            ...
            for symbol, recovery in self._recovery_managers.items():
                ...
                try:
                    ...
                    async with session.get(rest_url, params=params) as resp:
                        ...
                        if gap <= 0:
                            logger.info(
                                f"No gap for {symbol}: last_trade_id={recovery._last_trade_id}"
                            )
                            continue
...
class PipelineManager:
    ...
    def get_all_statuses(self) -> dict[str, dict[str, Any]]:
        ...
        """
        return {pid: pipeline.get_status() for pid, pipeline in self._pipelines.items()}
