...
class DataQualityGuard:
    ...
    OPTIONAL_INDICATORS = {
        'atr_999', 'ema_450', 'sma_450', 'ema_2000', 'sma_2000',
        'macd_120_260_29_macd', 'macd_120_260_29_signal', 'macd_120_260_29_histogram',
        'macd_400_860_300_macd', 'macd_400_860_300_signal', 'macd_400_860_300_histogram',
        'bb_900_2_lower', 'bb_900_2_upper', 'bb_900_2_middle',
    }

    # Indicators that should NEVER be null (critical indicator backbone)
    CRITICAL_INDICATORS = {
        'atr_14', 'atr_99', 'ema_12', 'ema_26', 'rsi_14',
        'sma_20', 'bb_20_2_std', 'bb_20_2_lower', 'bb_20_2_upper',
        'bb_20_2_middle', 'macd_12_26_9_macd', 'macd_12_26_9_signal',
    ...
    def _validate_indicator(
        ...
        if value is None:
            ...
            self.issues.append(DataIssue(
                ...
                value=value,
                severity=severity,
                message=f'{key} is null' + (' (critical indicator)' if severity == 'error' else '')
            ))
            return
        ...
        if value < 0:
            ...
            negative_allowed = any(p in key.lower() for p in [
                'macd', 'histogram', 'change', 'diff', 'momentum'
            ])
    ...
    def get_issue_summary(self, reports: List[QualityReport]) -> Dict:
        ...
        for report in reports:
            for issue in report.issues:
                ...
                summary['issues_by_type'][key] = \
                    summary['issues_by_type'].get(key, 0) + 1

                # By severity
                key = issue.severity
                summary['issues_by_severity'][key] = \
                    summary['issues_by_severity'].get(key, 0) + 1

                # By indicator
                key = issue.indicator
                summary['affected_indicators'][key] = \
                    summary['affected_indicators'].get(key, 0) + 1
    ...
    def validate_batch(
        ...
        symbol_id: int,
        symbol: str,
        time_value_pairs: List[tuple[datetime, Dict[str, Any]]]
    ) -> List[QualityReport]:
