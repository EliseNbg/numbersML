"""Algorithm config schema loading and validation helpers."""
...
def _schema_path() -> Path:
    return Path(__file__).parent / "schemas" / "algorithm_config_v1.json"


def load_algorithm_schema() -> dict[str, Any]:
    """Load Algorithm Config V1 JSON schema from disk."""
    return json.loads(_schema_path().read_text(encoding="utf-8"))


def validate_algorithm_config(config: dict[str, Any]) -> tuple[bool, list[ValidationIssue]]:
    """Validate algorithm config against schema and business rules."""
    validator = Draft202012Validator(load_algorithm_schema())
    issues = [
        ValidationIssue(
        ...
        if not isinstance(rules, list) or len(rules) == 0:
            issues.append(
                ValidationIssue(
                    ...
                    message="composite algorithms require non-empty rules list.",
                )
            )
    return issues
        ...
        if key not in params:
            issues.append(
                ValidationIssue(
                    ...
                    message=f"{key} is required for rsi algorithm.",
                )
            )
    ...
    for key in required_keys:
        if key not in params:
            issues.append(
                ValidationIssue(
                    path=f"signal.params.{key}",
                    message=f"{key} is required for macd algorithm.",
                )
            )
    ...
    for key in required_keys:
        if key not in params:
            issues.append(
                ValidationIssue(
                    path=f"signal.params.{key}",
                    message=f"{key} is required for sma_cross algorithm.",
                )
            )
    ...
    for key in required_keys:
        if key not in params:
            issues.append(
                ValidationIssue(
                    path=f"signal.params.{key}",
                    message=f"{key} is required for bollinger algorithm.",
                )
            )
