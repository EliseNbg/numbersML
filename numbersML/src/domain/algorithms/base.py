"""
Algorithm Interface - Base class and types for trading algorithms.

Provides the foundation for implementing trading algorithms that
...
class Signal:
    """
    Trading signal generated by algorithm.
...
class Algorithm(ABC):
    ...
    Purpose:
        Defines the interface for all trading algorithms.
        Algorithms consume enriched tick data via Redis pub/sub
    ...
    def __init__(
        ...
        Raises:
            ValueError: If algorithm_id or symbols is empty
        """
        if not algorithm_id:
        ...
        self._algorithm_id: UUID = algorithm_id
        self._symbols: list[str] = symbols
        self._time_frame: TimeFrame = time_frame

        logger.info(f"Algorithm {algorithm_id} initialized for {len(symbols)} symbols")
    ...
    def on_tick(self, tick: EnrichedTick) -> Signal | None:
        ...
        This is the main algorithm logic method.
...
class AlgorithmManager:
    ...
    def list_instances(self) -> list[UUID]:
        """Get list of algorithm instance IDs."""
    ...
    async def start_all(self) -> None:
        """Start all algorithm instances."""
        logger.info(f"Starting {len(self._instances)} algorithm instances")
        self._running = True
    ...
    async def stop_all(self) -> None:
        """Stop all algorithm instances."""
        logger.info("Stopping all algorithm instances")
        self._running = False
    ...
    def process_tick(self, tick: EnrichedTick) -> list[Signal]:
        """
        Process tick through all algorithm instances.
    ...
    def get_all_signals(self) -> list[Signal]:
        """Get all signals from all algorithm instances."""
        signals = []
        for instance in self._instances.values():
    ...
    def get_stats(self) -> dict[str, Any]:
        """Get statistics for all algorithm instances."""
        return {
