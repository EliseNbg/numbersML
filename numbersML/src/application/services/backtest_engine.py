...
class BacktestMetricsCalculator:
    ...
    def calculate(
        ...
        if duration_days > 0:
            ...
            metrics.cagr = ((final_equity / initial_balance) ** (1 / years) - 1) * 100 if years > 0 else 0
        ...
        if len(equity_values) > 1:
            ...
            if metrics.volatility > 0:
                metrics.sharpe_ratio = (avg_return * periods_per_year) / metrics.volatility_annualized
...
class BacktestEngine:
        ...
        for i, candle in enumerate(candles):
            ...
            if signal:
                ...
                if signal.signal_type == SignalType.BUY and not positions.get(signal.symbol):
                    # Open long position
                    cash, pos, trade = self._open_position(
                        signal, candle, cash, config
                    )
                    if pos:
    ...
    def _open_position(
        ...
        position = {
            ...
            'stop_loss': risk_config.get('stop_loss_pct') and executed_price * (1 - risk_config.get('stop_loss_pct', 0) / 100),
            'take_profit': risk_config.get('take_profit_pct') and executed_price * (1 + risk_config.get('take_profit_pct', 0) / 100),
        }
