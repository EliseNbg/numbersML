...
from src.application.services.strategy_loader import ConfigDrivenStrategy
...
@dataclass
class DebugMessage:
    """Structured backtest debug message."""
...
class BacktestResult:
    ...
    debug_messages: list[DebugMessage] = field(default_factory=list)
...
class BacktestMetricsCalculator:
    ...
    def calculate(
        ...
        win_durations: list[timedelta] = []
        loss_durations: list[timedelta] = []
        ...
        if equity_curve:
            ...
            exposures = [
                ...
                for pos_val, equity in zip(position_values, equities, strict=False)
            ]
...
class BacktestEngine:
    ...
    async def run_backtest(
        ...
        strategy = strategy_instance or self._build_strategy_from_config(
                ...
                if signal:
                    debug_messages.append(
                        DebugMessage(
                            ...
                            level="DEBUG",
                            message=(
                                f"Signal {signal.signal_type.value} {signal.symbol} at "
                                f"{float(signal.price):.4f} metadata={signal.metadata}"
                            ),
                        )
                    )
                        ...
                        if pos:
                            ...
                            debug_messages.append(
                                DebugMessage(
                                    ...
                                    level="INFO",
                                    message=(
                                        f"Opened {signal.symbol} long quantity={pos['quantity']:.8f} "
                                        f"at {pos['entry_price']:.4f}"
                                    ),
                                )
                            )
    ...
    def _close_position(
        ...
        pnl = net_proceeds - entry_cost - position.get("entry_fees", 0.0)
        ...
        trade = TradeRecord(
            ...
            pnl=pnl,
            pnl_pct=pnl_pct,
            fees=fees + position.get("entry_fees", 0.0),
    ...
    def _build_strategy_from_config(
        ...
        """Build a fallback config-driven strategy when no instance is supplied."""
...
def serialize_trade_record(trade: TradeRecord) -> dict[str, Any]:
    """Convert a trade record into a JSON-serializable payload."""
    return {
...
def serialize_equity_point(point: EquityPoint) -> dict[str, Any]:
    """Convert an equity curve point into a JSON-serializable payload."""
    return {
...
def serialize_debug_message(message: DebugMessage) -> dict[str, Any]:
    """Convert a debug message into a JSON-serializable payload."""
    return {
...
def serialize_metrics(metrics: BacktestMetrics) -> dict[str, Any]:
    """Convert metrics into a JSON-serializable payload."""
    payload = asdict(metrics)
    for key, value in list(payload.items()):
        if isinstance(value, timedelta):
            payload[key] = value.total_seconds()
    return payload
