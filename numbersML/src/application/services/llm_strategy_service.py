"""
LLM Strategy Service - AI-assisted strategy configuration generation.

Provides safe, guardrailed LLM integration for:
- Generating strategy configs from natural language
...
class LLMStrategyService:
    """
    LLM-assisted strategy configuration service.

    Responsibilities:
    1. Generate strategy configs from natural language descriptions
    ...
    def __init__(
        ...
        Args:
            strategy_repository: Repository for saving generated strategies
        ...
        if OPENAI_AVAILABLE:
            ...
            if key:
                ...
                logger.info(f"LLMStrategyService initialized with model {model}")
            else:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "You are a trading strategy configuration expert. "
                            "Generate valid JSON strategy configurations only. "
    ...
    async def modify_config(
        ...
        """
        Modify existing strategy config based on change request.

        Args:
            existing_config: Current strategy configuration
        ...
        try:
                ...
                messages=[
                    {
                        ...
                        "content": (
                            "You are a trading strategy configuration expert. "
                            "Modify the provided strategy config based on the user's request. "
            ...
            # Schema validation
            is_valid, validation_issues = validate_strategy_config(config)
    ...
    async def suggest_improvements(
        ...
        """
        Suggest strategy improvements based on backtest performance.
        ...
        try:
            prompt = self._build_suggestion_prompt(strategy_config, backtest_metrics)
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Provide concise, actionable recommendations for strategy improvement. "
    ...
    async def save_generated_strategy(
        ...
        """
        Save LLM-generated config as a new draft strategy.
        ...
        Returns:
            Created StrategyDefinition (always as draft)
        """
        ...
        strategy_def = StrategyDefinition(
            name=meta.get("name", "Untitled LLM Strategy"),
        ...
        # Save to repository
        saved = await self._repository.save(strategy_def)
        ...
        logger.info(f"Saved LLM-generated strategy as draft: {saved.id}")
        return saved
    ...
    def _build_generation_prompt(
        ...
        return f"""Generate a trading strategy configuration based on the following description.
...
Output JSON only, no markdown, no explanations."""
    ...
    def _build_modification_prompt(
        ...
        return f"""Modify the following strategy configuration based on the user's request.
...
Output the complete modified configuration as JSON:"""
    ...
    def _build_suggestion_prompt(
        ...
        return f"""Analyze this strategy's backtest performance and suggest improvements.
