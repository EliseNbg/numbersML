...
class RiskGuardrailService:
    ...
    def get_global_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "global_kill_time": self._global_kill_time.isoformat() if self._global_kill_time else None,
    ...
    async def _trigger_breach(
        ...
        if action == GuardrailAction.PAUSE_STRATEGY and strategy_id:
            ...
            if state:
                ...
                logger.warning(f"Strategy {strategy_id} paused for {cooldown}m due to {guardrail_type.value}")
        
        elif action == GuardrailAction.EMERGENCY_STOP:
            if strategy_id:
                await self.trigger_strategy_kill(strategy_id, f"Guardrail breach: {guardrail_type.value}")
            else:
