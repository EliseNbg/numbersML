...
from .risk_guardrails import RiskGuardrailService, get_risk_guardrail_service
from .audit_logger import AuditLogger, AuditSeverity, get_audit_logger
...
class EmergencyStopService:
    ...
    async def emergency_stop(
        ...
        logger.critical(
            f"EMERGENCY STOP TRIGGERED: {level.value} by {triggered_by}: {reason}"
        )
    ...
    async def release_stop(
        ...
        logger.critical(
            f"EMERGENCY STOP RELEASED: {stop_id} by {released_by}: {reason}"
        )
