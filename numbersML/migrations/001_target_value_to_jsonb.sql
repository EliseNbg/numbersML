-- Migration: Convert target_value from double precision to JSONB
-- This migration:
-- 1. Creates new JSONB column
-- 2. Migrates existing float values (sets trend='unknown' as placeholder)
-- 3. Drops old column and renames new one
-- 
-- IMPORTANT: After running this, you MUST recalculate target data:
--   python3 -m src.cli.recalculate_targets --all
-- This will run the Kalman filter on all candles and populate proper JSONB values

-- Step 1: Add new JSONB column
ALTER TABLE candles_1s ADD COLUMN IF NOT EXISTS target_data JSONB;

-- Step 2: Migrate existing data (placeholder - needs recalculation)
UPDATE candles_1s
SET target_data = jsonb_build_object(
    'filtered_value', target_value,
    'close', close,
    'diff', 0,
    'trend', 'unknown',
    'velocity', 0
)
WHERE target_value IS NOT NULL AND target_data IS NULL;

-- Step 3: Drop old column
ALTER TABLE candles_1s DROP COLUMN IF EXISTS target_value;

-- Step 4: Rename new column to target_value
ALTER TABLE candles_1s RENAME COLUMN target_data TO target_value;

-- Step 5: Add GIN index for JSONB queries
CREATE INDEX IF NOT EXISTS idx_candles_1s_target_trend
ON candles_1s USING gin (target_value)
WHERE (target_value->>'trend') IS NOT NULL;

-- Step 6: Add index for trend queries
CREATE INDEX IF NOT EXISTS idx_candles_1s_target_trend_btree
ON candles_1s ((target_value->>'trend'))
WHERE (target_value->>'trend') != 'unknown';

-- Verify migration
SELECT
    COUNT(*) as total_candles,
    COUNT(target_value) as candles_with_target,
    target_value->>'trend' as trend,
    COUNT(*) as trend_count
FROM candles_1s
WHERE target_value IS NOT NULL
GROUP BY target_value->>'trend'
ORDER BY trend_count DESC
LIMIT 10;

-- Note: All candles will show trend='unknown' until recalculated
-- Run: python3 -m src.cli.recalculate_targets --all
