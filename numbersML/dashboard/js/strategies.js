 ...
 *
 * Handles:
 * - Strategy CRUD operations (config-based and class-based)
...
function bindEventListeners() {
    ...
    document.getElementById('strategy-type').addEventListener('change', function () {
        const isClassBased = this.value === 'class';
        document.getElementById('class-based-section').classList.toggle('d-none', !isClassBased);
        document.getElementById('config-based-section').classList.toggle('d-none', isClassBased);
    });

    // Class selection change
    document.getElementById('strategy-class-select').addEventListener('change', onClassSelected);

    // Action buttons
    document.getElementById('btn-create-strategy').addEventListener('click', () => {
        loadConfigTemplate('rsi');
        loadUserStrategyClasses();
        createModal.show();
    });
...
/**
 * Load user strategy classes from API
 */
async function loadUserStrategyClasses() {
    try {
        const response = await fetch(`${API_BASE_URL}/strategies/user-classes`);
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}`);
        }
        userStrategyClasses = await response.json();
        populateClassDropdown();
    } catch (error) {
        console.error('Failed to load user strategy classes:', error);
        showAlert('warning', `Could not load strategy classes: ${error.message}`);
    }
}
...
function populateClassDropdown() {
    ...
    select.innerHTML = '<option value="">Select a strategy class...</option>';

    userStrategyClasses.forEach(cls => {
        const option = document.createElement('option');
        option.value = cls.class_path;
        option.textContent = `${cls.class_name} (${cls.module})`;
        option.dataset.docstring = cls.docstring || '';
        option.dataset.hasOnTick = cls.has_on_tick;
...
function onClassSelected() {
    const select = document.getElementById('strategy-class-select');
    const metadataDiv = document.getElementById('class-metadata');
    const docstringP = document.getElementById('class-docstring');
    const detailsP = document.getElementById('class-details');

    if (!select.value) {
        metadataDiv.classList.add('d-none');
        return;
    }

    const selectedOption = select.options[select.selectedIndex];
    const docstring = selectedOption.dataset.docstring;
    const hasOnTick = selectedOption.dataset.hasOnTick === 'true';

    docstringP.textContent = docstring || 'No documentation available.';
    detailsP.innerHTML = `
        <strong>Module:</strong> ${select.value}<br>
        <strong>Has on_tick:</strong> <span class="badge bg-${hasOnTick ? 'success' : 'warning'}">${hasOnTick ? 'Yes' : 'No'}</span>
    `;
...
function getSignalType(strategy) {
    if (strategy.strategy_type === 'class') {
        return '<span class="badge bg-primary">Class-Based</span>';
    }
    return '<span class="badge bg-secondary">Config-Based</span>';
}
...
function getSymbols(strategy) {
    if (strategy.symbols && strategy.symbols.length > 0) {
        return strategy.symbols.map(s => `<span class="badge bg-light text-dark me-1">${escapeHtml(s)}</span>`).join('');
    }
...
/**
 * Create new strategy (supports both config-based and class-based)
 */
async function createStrategy() {
    ...
    if (strategyType === 'class') {
        // Class-based strategy
        const classPath = document.getElementById('strategy-class-select').value;
        if (!classPath) {
            showAlert('warning', 'Please select a strategy class');
            return;
        }
        payload.strategy_type = 'class';
...
/**
 * Open modify modal with current config
 */
async function openModifyModal() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const activeVersion = versions.find(v => v.is_active) || versions[versions.length - 1];
            if (activeVersion) {
                document.getElementById('modify-config').value = JSON.stringify(activeVersion.config, null, 2);
            }
        }
    } catch (error) {
        showAlert('danger', `Failed to load config: ${error.message}`);
    }
...
/**
 * Load market status (balance, positions, orders)
 */
...
async function loadBalance() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/market/balance`);
        if (response.ok) {
            ...
            const html = balances
                ...
                .map(b => `
                    <div class="d-flex justify-content-between">
                        ...
                        <span>Free: ${b.free} | Locked: ${b.locked}</span>
                    </div>
                `).join('');
            widget.innerHTML = html || '<p class="text-muted mb-0">No balance found</p>';
        } else {
            widget.innerHTML = '<p class="text-muted mb-0">Balance not available</p>';
        }
    } catch (error) {
        widget.innerHTML = '<p class="text-danger mb-0">Failed to load balance</p>';
    }
}
...
async function loadPositions() {
    const widget = document.getElementById('positions-widget');
    try {
        const response = await fetch(`${API_BASE_URL}/market/positions`);
        if (response.ok) {
            const positions = await response.json();
            if (positions.length === 0) {
                widget.innerHTML = '<p class="text-muted mb-0">No open positions</p>';
                return;
            }
            widget.innerHTML = positions.map(p => `
                <div class="border-bottom pb-2 mb-2">
                    <div class="d-flex justify-content-between">
                        <strong>${p.symbol}</strong>
                        <span class="badge bg-${p.side === 'LONG' ? 'success' : 'danger'}">${p.side}</span>
                    </div>
                    <small class="text-muted">
                        Size: ${p.size} | Entry: ${p.entry_price} | PnL: ${p.unrealized_pnl}
                    </small>
                </div>
            `).join('');
        } else {
            widget.innerHTML = '<p class="text-muted mb-0">Positions not available</p>';
        }
    } catch (error) {
        widget.innerHTML = '<p class="text-danger mb-0">Failed to load positions</p>';
    }
}
...
async function loadOrders() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/market/orders?limit=5`);
        if (response.ok) {
            const orders = await response.json();
            if (orders.length === 0) {
                widget.innerHTML = '<p class="text-muted mb-0">No recent orders</p>';
                return;
            }
            widget.innerHTML = orders.map(o => `
                <div class="border-bottom pb-2 mb-2">
                    <div class="d-flex justify-content-between">
                        <strong>${o.symbol}</strong>
                        <span class="badge bg-${o.side === 'BUY' ? 'success' : 'danger'}">${o.side}</span>
                    </div>
                    <small class="text-muted">
                        ${o.status} | ${o.type} | ${o.price || '-'} | Qty: ${o.quantity}
                    </small>
                </div>
            `).join('');
        } else {
            widget.innerHTML = '<p class="text-muted mb-0">Orders not available</p>';
        }
    } catch (error) {
        widget.innerHTML = '<p class="text-danger mb-0">Failed to load orders</p>';
    }
}
